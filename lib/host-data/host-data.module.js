    ...
    controller: ['storage', '$scope', function HostDataController(storage, $scope) {
      ...
      self.addRule = function($event) {
        if (self.newRule.hostName.length > 0 && self.newRule.ip.length > 0) {
          var existingHost = self.data.find(host => host.hostName === self.newRule.hostName && !!host.incognito == chrome.extension.inIncognitoContext);
          if (!existingHost) {
            self.data.push({
      ...
      self.saveRules = function() {
        storage.save(self.data.map(rule => {
